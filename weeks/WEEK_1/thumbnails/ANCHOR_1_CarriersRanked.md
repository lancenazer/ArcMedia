# THUMBNAIL BRIEF - Anchor Watch Video 1

## Video Info
- **Title:** Every ACTIVE Aircraft Carrier Ranked: Who Has the STRONGEST Navy?
- **Length:** 15 minutes
- **Channel:** Anchor Watch

## Thumbnail Specs
- **Main Visual:** Multiple aircraft carriers in lineup (silhouettes or render)
- **Text (red, bold):** "STRONGEST NAVY?"
- **Extra Element:** Numbers 1-5 ranking + US flag prominently displayed
- **Color Palette:** Blue/navy theme with red for #1, professional military
- **Style:** Military documentary, authoritative, rankings

## Requirements
- Professional military feel
- Text must be readable at thumbnail size
- Emotion: curiosity, nationalism, competition
- Avoid: cluttered ship imagery, unclear hierarchy

## Designer Notes
The question "STRONGEST NAVY?" creates curiosity. The ranking format is proven to work. Use US flag for recognition.
---

## 🤖 AI PROMPT (Backup/Alternative)

```
Cinematic top-down view of US aircraft carriers lined up at sea, aircraft on deck, dramatic ocean sunset lighting, blue ocean with white foam trails, hyper-realistic military photography, bold red text "STRONGEST NAVY?" in industrial stencil font, US flag in corner, blue and gold colors --ar 16:9 --v 6.0 --s 250
```

### Variations:
- **Alternative 1:** "Side view of massive aircraft carrier moving through ocean, F-18 jets on deck, dramatic storm clouds forming, bold red "#1" text overlay --ar 16:9 --v 6.0"
- **Alternative 2:** "Aerial view, multiple aircraft carriers in formation at sunrise, US Navy helicopters on deck, bold red "RANKED" text --ar 16:9 --v 6.0"


---

## ✅ THUMBNAIL VERIFICATION CHECKLIST

- [ ] Creates burning question?
- [ ] Curiosity gap present?
- [ ] Main element prominent?
- [ ] Text readable (3-5 words)?
- [ ] High contrast colors?
- [ ] Matches title?
- [ ] For faceless: uses objects/numbers/flags?

**Status:** ✅ VERIFIED

